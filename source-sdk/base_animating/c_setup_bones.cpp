...
public:
	void Construct()
	{
		...
		static auto ik_ctor = (IKConstruct)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("53 8B D9 F6 C3 03 74 0B FF 15 ?? ?? ?? ?? 84 C0 74 01 CC C7 83 ?? ?? ?? ?? ?? ?? ?? ?? 8B CB"));
	...
	void Destructor()
	{
		...
		static auto ik_dector = (IKDestructor)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("56 8B F1 57 8D 8E ?? ?? ?? ?? E8 ?? ?? ?? ?? 8D 8E ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 BE ?? ?? ?? ?? ??"));
	...
	void Init(c_studio_hdr* hdr, vec3_t* angles, vec3_t* origin, float currentTime, int frames, int boneMask)
	{
		using Init_t = void(__thiscall*)(void*, c_studio_hdr*, qangle_t*, vec3_t*, float, int, int);

		static auto ik_init = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 EC 08 8B 45 08 56 57 8B F9 8D 8F"));
		((Init_t)ik_init)(this, hdr, (qangle_t*)angles, origin, currentTime, frames, boneMask);
	}

	void UpdateTargets(vec3_t* pos, Quaternion* qua, matrix3x4_t* matrix, uint8_t* boneComputed)
	{
		using UpdateTargets_t = void(__thiscall*)(void*, vec3_t*, Quaternion*, matrix3x4_t*, uint8_t*);

		static auto ik_update_targets = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F0 81 EC ? ? ? ? 33 D2 89 4C 24 34"));
		((UpdateTargets_t)ik_update_targets)(this, pos, qua, matrix, boneComputed);
	}

	void SolveDependencies(vec3_t* pos, Quaternion* qua, matrix3x4_t* matrix, uint8_t* boneComputed)
	{
		using SolveDependencies_t = void(__thiscall*)(void*, vec3_t*, Quaternion*, matrix3x4_t*, uint8_t*);

		static auto ik_solve_dependencies = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F0 81 EC ? ? ? ? 8B 81"));
...
struct CBoneSetup
{
	...
	void InitPose(vec3_t pos[], Quaternion q[], c_studio_hdr* hdr)
	{
		static auto init_pose = utilities::pattern_scan("client.dll", "55 8B EC 83 EC 10 53 8B D9 89 55 F8 56 57 89 5D F4 8B 0B 89 4D F0");
	...
	void AccumulatePose(vec3_t pos[], Quaternion q[], int sequence, float cycle, float flWeight, float flTime, CIKContext* pIKContext)
	{
		using AccumulatePoseFn = void(__thiscall*)(CBoneSetup*, vec3_t* a2, Quaternion* a3, int a4, float a5, float a6, float a7, CIKContext* a8);
		static auto accumulate_pose = (AccumulatePoseFn)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F0 B8 ? ? ? ? E8 ? ? ? ? A1"));

		return accumulate_pose(this, pos, q, sequence, cycle, flWeight, flTime, pIKContext);
	}

	void CalcAutoplaySequences(vec3_t pos[], Quaternion q[], float flRealTime, CIKContext* pIKContext)
	{
		static auto calc_autoplay_sequences = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 EC 10 53 56 57 8B 7D 10 8B D9 F3 0F 11 5D ??"));
	...
	void CalcBoneAdj(vec3_t pos[], Quaternion q[], float* controllers, int boneMask)
	{
		static auto calc_bone_adj = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F8 81 EC ?? ?? ?? ?? 8B C1 89 54 24 04 89 44 24 2C 56 57 8B 00"));
...
{
	static auto bone_merge = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("89 86 ?? ?? ?? ?? E8 ?? ?? ?? ?? FF 75 08"));
	return *(uintptr_t*)((uintptr_t)player + *(uintptr_t*)(bone_merge + 0x2));
}



mstudioposeparamdesc_t* pPoseParameter(c_studio_hdr* hdr, int index)
{
	using poseParametorFN = mstudioposeparamdesc_t * (__thiscall*)(c_studio_hdr*, int);
	static poseParametorFN pose_parameter = (poseParametorFN)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 8B 45 08 57 8B F9 8B 4F 04 85 C9 75 15 8B"));
...
{
	static auto bone_merge_update_cache = (void(__thiscall*)(uintptr_t))rel32_fix((std::uint32_t)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("E8 ?? ?? ?? ?? 83 7E 10 00 74 64")));
	bone_merge_update_cache(bonemerge);
}

float GetPoseParamValue(c_studio_hdr* hdr, int index, float flValue)
{
	if (index < 0 || index > 24)
		return 0.0f;
	...
	if (PoseParam.m_loop)
	{
		auto wrap = (PoseParam.m_start + PoseParam.m_end) / 2.0f + PoseParam.m_loop / 2.0f;
		auto shift = PoseParam.m_loop - wrap;

		flValue = flValue - PoseParam.m_loop * floor((flValue + shift) / PoseParam.m_loop);
	}

	return (flValue - PoseParam.m_start) / (PoseParam.m_end - PoseParam.m_start);
}
		...
		do
		{
			if (*index != -1)
			{
				...
				if (hdr && *(studiohdr_t**)hdr && counter >= 0)
				{
					...
					pose_param_value = pose * (pose_param->m_end - pose_param->m_start) + pose_param->m_start;
				}
...
{
	static auto ik = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("8B 8F ?? ?? ?? ?? 89 4C 24 1C"));
	auto m_pIk = (CIKContext*)((uintptr_t)m_animating->get_renderable_virtual() + 0x99B0);
	...
	if (m_pIk)
	{
		m_pIk->Init(m_pHdr, &m_angAngles, &m_vecOrigin, m_flCurtime, math::time_to_ticks(m_flCurtime), m_boneMask);
	...
	static auto invalidate_bone_cache = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("80 3D ?? ?? ?? ?? ?? 74 16 A1 ?? ?? ?? ?? 48 C7 81"));
...
{
	static auto custom_player = *(int*)(utilities::pattern_scan(crypt_str("client.dll"), crypt_str("80 BF ?? ?? ?? ?? ?? 0F 84 ?? ?? ?? ?? 83 BF ?? ?? ?? ?? ?? 74 7A")) + 2);

	if (!*(bool*)((uintptr_t)player + custom_player) || !player->get_anim_state())
		return false;
	...
	auto world_model = (player_t*)weapon->model();

	if (!world_model || *(short*)((uintptr_t)world_model + 0x26E) == -1)
		return player == local_pointer;
...
{
	alignas(16) vec3_t position[256];
	...
	static auto copy_to_follow = rel32_fix((std::uint32_t)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("E8 ?? ?? ?? ?? 8B 87 ?? ?? ?? ?? 8D 8C 24 ?? ?? ?? ?? 8B 7C 24 18")));
	static auto bone_merge_copy_to_follow = (void(__thiscall*)(uintptr_t, vec3_t*, Quaternion*, int, vec3_t*, Quaternion*))copy_to_follow;

	static auto copy_from_follow = rel32_fix((std::uint32_t)utilities::pattern_scan(crypt_str("client.dll"), crypt_str("E8 ?? ?? ?? ?? F3 0F 10 45 ?? 8D 84 24 ?? ?? ?? ??")));
	static auto bone_merge_copy_from_follow = (void(__thiscall*)(uintptr_t, vec3_t*, Quaternion*, int, vec3_t*, Quaternion*))copy_from_follow;

	char tmp_buffer[4208];
	auto world_ik = (CIKContext*)tmp_buffer;

	auto weapon = m_animating->active_weapon();
		...
		if (weaponWorldModel)
		{
			...
			if (bone_merge)
			{
				...
				for (auto i = 0; i < m_nAnimOverlayCount; ++i)
				{
					...
					{
						if (m_bShouldDispatch && m_animating == local_pointer)
						{
							using UpdateDispatchLayer = void(__thiscall*)(void*, animationlayer*, c_studio_hdr*, int);
							call_virtual <UpdateDispatchLayer>(m_animating, 246 + 1)(m_animating, layer, world_hdr, layer->m_nSequence);
						}

						if (!m_bShouldDispatch || layer->m_nDispatchSequence_2 <= 0 || layer->m_nDispatchSequence_2 >= world_hdr->m_pStudioHdr->local_seq_count)
						...
						else if (m_bShouldDispatch)
						{
							...
							static auto add_dependencies = utilities::pattern_scan(("client.dll"), ("55 8B EC 81 EC BC 00 00 00 53 56 57"));
...
void CSetupBones::studio_build_matrices(c_studio_hdr* hdr, const matrix3x4_t& rotationmatrix, vec3_t* pos, Quaternion* q, int boneMask, matrix3x4_t* bonetoworld, uint32_t* boneComputed)
{
	auto i = 0;
	auto chain_length = 0;
	auto bone = -1;
	auto studio_hdr = hdr->m_pStudioHdr;

	if (bone < -1 || bone >= studio_hdr->bones_count)
		bone = 0;

	auto bone_parent = (c_utl_vector <int>*) ((uintptr_t)hdr + 0x44);
	auto bone_flags = (c_utl_vector <int>*) ((uintptr_t)hdr + 0x30);

	int chain[MAXSTUDIOBONES];

	if (bone <= -1)
	{
		chain_length = studio_hdr->bones_count;

		for (i = 0; i < studio_hdr->bones_count; ++i)
	...
	for (auto j = chain_length - 1; j >= 0; --j)
	{
		...
		auto flag = bone_flags->m_memory[i];
		auto parent = bone_parent->m_memory[i];
...
{
	using AttachmentHelperFn = void(__thiscall*)(player_t*, c_studio_hdr*);
	static auto m_AttachmentHelper = utilities::pattern_scan(crypt_str("client.dll"), crypt_str("55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4"));
...
void CSetupBones::fix_bones_rotations()
{
	...
	if (studio_hdr)
	{
		auto hdr = studio_hdr->m_pStudioHdr;

		if (hdr)
		{
			auto hitbox_set = hdr->hitbox_set(m_animating->hitbox_set());

			for (auto i = 0; i < hitbox_set->hitbox_count; ++i)
			{
				auto hitbox = hitbox_set->hitbox(i);

				if (hitbox->rotation.is_zero())
					continue;
