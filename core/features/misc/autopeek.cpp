...
namespace autopeek {
	...
	void run_cm ( c_usercmd * cmd ) {
		...
		if ( m_data.finished )
			return;
		m_data.origin = local_player::m_data.pointer->origin ( );
	...
	void search_position ( ) {
		...
		auto target = aimbot::targets.back ( );
		if ( !target.player )
			return;
		...
		*/
		...
		for ( float a = 0; a < ( M_PI * 2.0 ); a += step ) {
			for ( int i = 1; i <= radius_steps; i++ ) {
				...
				float dmg = autowall::get_damage ( local_player::m_data.pointer, location, target_head, awall );
				float dist = math::calc_distance ( position, tr.end, false );
				if ( dmg > 10.f ) {
					if ( dist < closest ) {
	...
	void move ( c_usercmd * cmd ) {
		const auto rotate_movement = [ & ] ( float yaw ) {
			float rotation = DEG2RAD ( localdata.orig_viewangle.y - yaw );
