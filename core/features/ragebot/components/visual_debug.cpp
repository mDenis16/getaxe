...
namespace aimbot {
	...
	void render ( ) {
		...
		for ( auto point : m_visual_debug.points ) {
			ImVec2 w2s;
			if ( visuals::world_to_screen (point.pos, w2s ) ) {
				c_menu::get ( ).draw->AddCircle ( w2s, 2.5f, point.safe ? ImColor ( 255, 221, 24, 255 ) : ImColor ( 255, 0, 0, 255 ) );
			}
		}
	}
