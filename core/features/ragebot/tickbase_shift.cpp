...
void tickbase_system::post_movement( ) {
	...
	if ( !csgo::local_player->is_alive() || !variables::ragebot::double_tap ) {
		return;
	}
  ...
  if ( index == weapon_revolver ||
	...
	bool bFastRecovery = true;

	float flNonShiftedTime = math::ticks_to_time( csgo::local_player->get_tick_base( ) - csgo::m_goal_shift );

	// Determine if we are able to shoot right now (at the time of the shift)
	bool bCanShootNormally = csgo::local_player->can_shoot_time( math::ticks_to_time( csgo::local_player->get_tick_base( ) ) );

	// Determine if we are able to shoot in the previous iShiftAmount ticks (iShiftAmount ticks before we shifted)
	bool bCanShootIn12Ticks = csgo::local_player->can_shoot_time( flNonShiftedTime );

	bool bIsShooting = csgo::local_player->is_firing( csgo::cmd, math::ticks_to_time( csgo::local_player->get_tick_base( ) ) );
	...
	if ( tickbase_system::m_shift_data.m_can_shift_tickbase  ) {
		...
		tickbase_system::m_shift_data.m_should_be_ready = false;
		variables::cheat::interval_per_tick = interfaces::globals->interval_per_tick;
	}
		...
		if ( bCanShootIn12Ticks ) {
			if ( tickbase_system::m_shift_data.m_prepare_recharge && !bIsShooting ) {
				...
				if ( bIsShooting ) {
					...
					tickbase_system::m_prediction.m_original_tickbase = csgo::local_player->get_tick_base( );
