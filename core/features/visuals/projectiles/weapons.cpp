...
namespace visuals::projectiles::weapons {
		...
		void think (weapon_t* weapon ) {
			if ( !config.visuals_world_draw_weapons )
				return;
		...
		void draw ( ) {
			for ( auto _data : weapons ) {
				if ( _data.on_screen ) {
				//	visuals::draw_text ( c_menu::get ( ).weapon_icons, _data.weapon_icon, ImVec2 ( ( _data.box_data.x + ( ( _data.box_data.x + _data.box_data.w ) - _data.box_data.x ) / 2.0f ), _data.box_data.y + _data.box_data.h + 4 ), 12, config.visuals_world_draw_weapons_color, 0.1f, true, true );
