...
static struct
{
    color window_border_fill{ 18, 18, 18, 255 };
    color window_border_color{ 18, 18, 18, 255 }; //main border
    color window_background{ 18, 18, 18, 255 };
    color filled_rectangle{ 153, 102, 204, 255 };

    color window_border_background{ 56, 56, 56, 255 };
    color control_outline{ 40, 40, 40, 255 };
    color control_active_or_clicked{ 153, 102, 204, 255 };
    color control_idle{ 18, 18, 18, 255 };

    color color_groupbox_bg{ 18, 18, 18, 255 };
    color color_text{ 203, 203, 203, 255 };
    color color_text_dimmer{ 203, 203, 203, 255 };
    color color_slider{ 153, 102, 204, 255 };
    color color_combo_bg{ 153, 102, 204, 255 };
} global_colors;
...
void zgui::poll_input( std::string_view window_name ) {
    ...
    int x, y;  interfaces::surface->surface_get_cursor_pos( x, y );
    POINT p_mouse_pos;
    p_mouse_pos.x = x; p_mouse_pos.y = y;
...
bool zgui::begin_window( std::string_view title, const vec2 default_size, const unsigned long font, const int flags ) {
    ...
    if ( !( flags & zgui_window_flags_always_open ) ) {
        if ( key_pressed( global_config.menu_toggle_key ) ) {
            ...
            variables::menu::opened = context.window.opened;
        }
    } else {
    ...
    if ( const int prev_alpha = context.window.alpha; !( flags & zgui_window_flags_no_ontoggle_animation ) ) {
        ...
        if ( context.window.alpha != prev_alpha ) {
            ...
            global_colors.filled_rectangle.a = context.window.alpha;
    ...
    if ( context.window.opened || context.window.alpha > 0 ) {
        ...
        if ( !( flags & zgui_window_flags_no_border ) ) {
            functions.draw_filled_rect( context.window.position.x - 0, context.window.position.y - 0, context.window.size.x + 0, context.window.size.y + 0, global_colors.window_border_color );
            functions.draw_filled_rect( context.window.position.x - 0, context.window.position.y - 0, context.window.size.x + 0, context.window.size.y + 0, global_colors.window_border_fill );
            ...
            functions.draw_filled_rect( context.window.position.x, context.window.position.y + 17, context.window.size.x, 1, global_colors.filled_rectangle );
        }

        if ( !( flags & zgui_window_flags_no_titlebar ) )
            functions.draw_text( context.window.position.x + context.window.size.x * 0.5, context.window.position.y + ( context.window.size.y * 0.010 ) - 2, global_colors.color_text, font, true, title.data( ) );
...
void zgui::colorpicker( const char* id, color& item ) noexcept
{
    ...
    int text_wide, text_tall;
    functions.get_text_size( render::fonts::menu_font, id_split [ 0 ].c_str( ), text_wide, text_tall );
...
bool zgui::tab_button( const char* id, const vec2 size, bool value ) noexcept {
    ...
    if ( value ) {
        context.window.render.emplace_back( zgui_control_render_t{ { draw_pos.x, draw_pos.y + size.y - 2 }, zgui_render_type::zgui_filled_rect, global_colors.control_active_or_clicked, "", {size.x - 1, 1} } );
    }
