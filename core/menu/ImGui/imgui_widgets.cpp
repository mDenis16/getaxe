...
bool ImGui::ButtonEx ( const char * label, const ImVec2 & size_arg, ImGuiButtonFlags flags ) {
    ...
    RenderFrame ( bb.Min, bb.Max, col, false, 5.2f );
...
bool ImGui::Hotkey ( const char * label, int * k, const ImVec2 & size_arg ) {
   ...
   // const bool focus_requested_by_code = focus_requested && ( window->fo == window->FocusIdxAllRequestCurrent );
...
bool ImGui::Checkbox ( const char * label, bool * v ) {
    ...
    RenderFrame ( check_bb.Min, check_bb.Max, GetColorU32 ( ( held && hovered ) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_CheckboxBackground ), true, style.FrameRounding );
...
bool ImGui::SliderScalar ( const char * label, ImGuiDataType data_type, void * p_data, const void * p_min, const void * p_max, const char * format, float power ) {
    ...
    const ImVec2 label_size = CalcTextSize ( label, NULL, true );
    // window->DC.CursorPos.x -= 17;
    const ImRect frame_bb ( ImVec2 ( window->DC.CursorPos.x, window->DC.CursorPos.y + 10 ), window->DC.CursorPos + ImVec2 ( w, label_size.y + style.FramePadding.y - 12.53f + 10 ) );
    const ImRect total_bb ( frame_bb.Min, frame_bb.Max + ImVec2 ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f ) );
    ...
    // Draw frame
    ImVec4 sadd { ImColor ( 151, 58, 255, 255 ) };
    ImVec4 sad_grabber { ImColor ( 151, 58, 15, 255 ) };

    const ImVec4 col_umplutura { ImColor ( 148,58,225 ) };
    const ImU32 col_umplutura_u32 = GetColorU32 ( col_umplutura );

    const ImVec4 frame_col_non { ImColor ( 52,51,53, 255 ) };
    const ImU32 frame_col = GetColorU32 ( frame_col_non );


    RenderNavHighlight ( frame_bb, id );
    RenderFrame ( frame_bb.Min, frame_bb.Max, frame_col, true, 15.6f );
    ...
    if ( value_changed )
        MarkItemEdited ( id );
    auto frame_bb_max_copy = frame_bb.Max;
    frame_bb_max_copy.x = grab_bb.GetCenter ( ).x;
    RenderFrame ( frame_bb.Min, frame_bb_max_copy, col_umplutura_u32, true, 15.6f );
    // Render grab
    if ( grab_bb.Max.x > grab_bb.Min.x )
        window->DrawList->AddCircleFilled ( grab_bb.GetCenter ( ), 10.f, GetColorU32 ( sadd ), 12 );

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
   // char value_buf [ 64 ];
  //  const char * value_buf_end = value_buf + DataTypeFormatString ( value_buf, IM_ARRAYSIZE ( value_buf ), data_type, p_data, format );

    // RenderTextClipped( grab_bb.GetCenter( ), grab_bb.GetCenter( ), value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));

   //  if (label_size.x > 0.0f)
    
    if ( data_type == ImGuiDataType_Float ) {
     auto   val = *( const float * ) p_data;
    // auto val_data = std::to_string ( ( int ) val ).c_str ( );

     ImVec2 text_val_size = CalcTextSize ( std::to_string ( ( int ) val ).c_str ( ), NULL, true );

     if ( text_val_size.x > 21 )
         text_val_size.x -= 15;
     else if ( text_val_size.x <= 11 )
         text_val_size.x += 15;
     RenderText ( ImVec2 ( frame_bb.Max.x + text_val_size.x, grab_bb.GetCenter ( ).y - 12.f ), std::to_string ( ( int ) val ).c_str ( ) );
    }
    else {
     auto   val = *( const int * ) p_data;
...
static bool InputTextFilterCharacter ( unsigned int * p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void * user_data ) {
    ...
    if ( c < 0x20 ) {
        bool pass = false;
        pass |= ( c == static_cast<int>('\n') && ( flags & ImGuiInputTextFlags_Multiline ) );
        pass |= ( c == static_cast< int >( '\t') && ( flags & ImGuiInputTextFlags_AllowTabInput ) );
        if ( !pass )
            return false;
    }
...
bool ImGui::ColorPicker4 ( const char * label, float col [ 4 ], ImGuiColorEditFlags flags, const float * ref_col ) {
    ...
    const float width = CalcItemWidth ( ) / 2;
    g.NextItemData.ClearFlags ( );
    ...
    float square_sz = GetFrameHeight ( ) / 2;
