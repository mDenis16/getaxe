...
bool __fastcall hooks::create_move::hook( void* ecx, void* edx, int input_sample_frametime, c_usercmd* cmd ) {
	...
	if ( interfaces::engine->get_net_channel ( ) )
		interfaces::console->console_printf ( "REAL angle %f choked = %i \n", cmd->viewangles.y, interfaces::engine->get_net_channel ( )->choked_packets );
...
const BYTE asdsadsadad [ ] = { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52,
...
void __fastcall hooks::update_client_side_animation::hook( player_t* player, uint32_t )
{
	...
	if ( player->index( ) == interfaces::engine->get_local_player( ) && player->simulation_time() != player->get_old_simulation_time() ) {
		auto o_angs = player->eye_angles ( );
	//	player->eye_angles ( ) = csgo::real_angle;
		...
		UpdateAnimationState222 ( player->get_anim_state ( ), csgo::real_angle );
	    update_client_side_animation_original( player );
		player->set_abs_angles ( vec3_t ( 0, csgo::real_angle, 0 ) );
	...
	//	csgo::local_player->set_local_viewangles( csgo::real_angle );
	   ...
	   // player->set_abs_angles ( vec3_t ( 0, resolver::server_feet_yaw(csgo::local_player), 0 ) );

//	   player->eye_angles ( ) = o_angs;
		...
		if ( variables::ragebot::resolver && !shot_this_time ) {
			...
			if ( resolver::resolver_data [ player->index ( ) ].brute_side == 1 )
				...
				feet_yaw -= resolver::resolver_data [ player->index ( ) ].brute_angle;
			//else
				//feet_yaw += resolver::resolver_data [ player->index ( ) ].first_shoot_correction;

	//		player->get_anim_state ( )->m_flGoalFeetYaw = feet_yaw;
			if ( resolver::resolver_data [ player->index ( ) ].brute_side > 0 )
			   player->set_abs_angles ( vec3_t ( 0, feet_yaw, 0.f ) );
		//	resolver::resolver_data [ player->index ( ) ].goal_feet_yaw = 0.f;
		}
...
bool __fastcall hooks::setup_bones::hook ( void * ecx, void * edx, matrix_t * bone_to_world_out, int max_bones, int bone_mask, float curtime ) {
	...
	*/
	...
	return setup_bones_original ( ecx, bone_to_world_out, max_bones, bone_mask, curtime );// math::get_estimate_server_time( csgo::cmd ) );
