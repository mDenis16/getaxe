...
#include "../hooks.hpp"
#include "../../../source-sdk/base_animating/c_setup_bones.h"
...
namespace hooks::callback
{
	...
	void __stdcall frame_stage_notify(client_frame_stage_t stage)
	{
		...
		local_player::m_data.pointer = reinterpret_cast<player_t*>(interfaces::entity_list->get_client_entity(
		...
		if (run && stage == FRAME_NET_UPDATE_POSTDATAUPDATE_START && local_pointer && localdata.in_game && local_pointer
			->is_alive())
		{
			...
			if (interfaces::inputsystem->is_button_down(KEY_C) && (next_update <= interfaces::globals->cur_time))
			{
				...
				{
					auto econItemView = localdata.active_weapon->GetEconItemView();
					if (econItemView)
					{
			...
			auto LocalPlayerInfo = localdata.pointer->get_info();
			...
			for (int nIndex = 0; hWeapons[nIndex]; nIndex++)
			{
				...
				auto econSkinData = LocalInventory->GetItemInLoadout(
					localdata.pointer->team(), weaponDefinition->GetEquippedPosition());
				if (!econSkinData)
					continue;
		...
		{
			/*update animations here to get before interpolation */
			anim_manager.on_frame_stage_before(stage);
		...
		if (local_player::available())
		{
			/*update animations here to get interpolated origin by game. ps: it uses uninterpolated data*/
			anim_manager.on_frame_stage_after(stage);
