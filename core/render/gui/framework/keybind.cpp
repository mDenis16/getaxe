...
namespace ui {
	...
	key_bind_muie::key_bind_muie(object* parent, config_manager::key_bind_item* key_bind) {
		...
		auto combo_test = new combobox(GET_STR("Key Mode"), this, std::vector<std::string>{"press", "toggle"}, * this->key_mode, 55.f);
		...
		case checkbox_element:
		{
			...
			new checkbox(GET_STR("New Value"), this, *(bool*)&this->new_value);
		}
		break;
		case slider_element:
		{
			auto parent = static_cast<slider*>(this->parrent);
			new slider(GET_STR("New Value"), this, *(float*)&this->new_value, *(float*)&parent->value_mins, *(float*)&parent->value_maxs, ui::slider_type::degrees);
		...
		case combobox_element:
		{
			...
			new combobox(GET_STR("New Value"), this, parent->items, *(int*)&this->new_value, 50.f);
