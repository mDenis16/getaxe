...
HRESULT c_font::draw_radial ( float x, float y, uint32_t color1, uint32_t color2, const wchar_t * text, uint8_t flags ) {
	...
	while ( *text ) {
		...
		if ( c != ' ' ) {
			const auto wv = x2 - x1;
			const auto hv = y2 - y1;
			const auto radius = max ( w, h ) / 2.f;
			const auto radius_vertex = max ( wv, hv ) / 2.f;
			const auto center = ImVec2 ( x - .5f + w / 2.f, y - .5f + h / 2.f );
			const auto center_vertex =  ImVec2 ( (x1  +  x2) / 2.f, ( y1 + y2 )  / 2.f);
			...
			clip.Width = min ( maxX - clip.X, static_cast< unsigned long >( w ) );
			clip.Height = min ( maxY - clip.Y, static_cast< unsigned long >( h ) );
