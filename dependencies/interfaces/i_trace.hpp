...
public:
	virtual int   get_point_contents( const vec3_t& abs_position, uintptr_t mask, void** pp_entity = nullptr ) = 0;
	virtual int   get_point_contents_world_only( const vec3_t& abs_position, uintptr_t mask ) = 0;
	virtual int   get_point_contents_collideable( collideable_t* collide, const vec3_t& abs_position ) = 0;
	virtual void  clip_ray_to_entity( const ray_t& ray, uintptr_t mask, player_t* entity, trace_t* trace ) = 0;
	virtual void  clip_ray_to_collideable( const ray_t& ray, uintptr_t mask, collideable_t* collide, trace_t* trace ) = 0;
	virtual void  trace_ray( const ray_t& ray, uintptr_t mask, i_trace_filter* trace_filter, trace_t* trace ) = 0;
};
